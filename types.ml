...
module SMap = struct
  ...
  let pp fv ppf map =
    Format.fprintf ppf "[ ";
    iter (fun k v -> Format.fprintf ppf "%d -> %a; " k fv v) map;
    Format.fprintf ppf " ]"
end
...
module Reason = struct
  type t = Conflict of Location.set * string * Location.set * string
...
module Components = struct
    ...
    | Object x, Object y, Pos ->
        Object
          (SMap.merge
             (fun k x y ->
               match (x, y) with
               (* lub takes intersection of fields *)
               | Some (lx, x), Some (ly, y) ->
                   Some (Location.join lx ly, f Pos x y)
               | _, _ -> None)
             x y)
    | Object x, Object y, Neg ->
        Object
          (SMap.merge
             (fun k x y ->
               match (x, y) with
               (* glb takes union of fields *)
               | Some (lx, x), Some (ly, y) ->
                   Some (Location.join lx ly, f Neg x y)
               | Some a, None | None, Some a -> Some a
               | None, None -> None)
             x y)
    | List (l, a), List (l', a'), pol -> List (Location.join l l', f pol a a')
    | Base (l, s), Base (l', s'), pol when s = s' -> Base (Location.join l l', s)
    | _, _, _ ->
        assert (cmp_component x y);
        assert false

  let lte pol f x y =
    match (x, y, pol) with
    | Func (l, d, r), Func (l', d', r'), pol -> f (polneg pol) d d' @ f pol r r'
        ...
        SMap.fold
          (fun k (yl, yk) r ->
            if SMap.mem k ox then
              ...
              [ Reason.Conflict (locations x, "{}", yl, Symbol.to_string k) ]
              @ r)
          oy []
        ...
        SMap.fold
          (fun k (xl, xk) r ->
            if SMap.mem k oy then
              ...
              [ Reason.Conflict (xl, Symbol.to_string k, locations y, "{}") ]
              @ r)
          ox []
    | List (l, a), List (l', a'), pol -> f pol a a'
    ...
    | x, y, pol -> (
        let name = function
          ...
          | Object _ -> "object"
          | List _ -> "list"
          | Base (_, s) -> Symbol.to_string s
        in
        let err a b =
          [ Reason.Conflict (locations a, name a, locations b, name b) ]
        in
  ...
  let list_fields = function
    | Func (l, d, r) -> [ ("d", d); ("r", r) ]
    | Object o ->
        List.map (fun (k, (_, v)) -> (Symbol.to_string k, v)) (SMap.bindings o)
    | List (l, a) -> [ ("e", a) ]
    | Base (l, s) -> []
  ...
  let print pr pol ppf = function
    ...
    | Object _ as o ->
        let rec pfield ppf = function
          | [] -> ()
          | [ (f, x) ] -> Format.fprintf ppf "%s :@ %a" f (pr pol) x
          | (f, x) :: xs ->
              Format.fprintf ppf "%s :@ %a,@ %a" f (pr pol) x pfield xs
        in
        Format.fprintf ppf "{%a}" pfield (list_fields o)
    | List (l, a) -> Format.fprintf ppf "%a list" (pr pol) a
    | Base (l, s) -> Format.fprintf ppf "%s" (Symbol.to_string s)
end
...
module TypeLat : TYPES = struct
  type 'a t = 'a Components.t list [@@deriving show]

  let lift t = [ t ]

  let pmap f pol ty = List.map (Components.pmap f pol) ty

  let rec pfold f pol ty x =
    match ty with
    | [] -> x
  ...
  let lte_pn f xs ys =
    ...
    List.fold_right
      (fun x rs ->
        List.fold_right
          (fun y rs -> Components.lte Pos (pol_flip f) x y @ rs)
          ys rs)
      xs []
    ...
    List.exists
      (fun x ->
        List.exists
          (fun y ->
            Components.lte Pos
              (fun p x y -> if pol_flip f p x y then [] else [ Reason.excuse ])
              x y
            = [])
          ys)
      xs
  ...
  let rec subs pol f xs ys =
    ...
    | x :: xs -> (
        ...
        | [ y ], ys ->
            Components.lte pol
              (fun p x y -> if f p x y then [] else [ Reason.excuse ])
              x y
            = []
            && subs pol f xs ys
        | y1 :: y2 :: _, _ -> failwith "two terms in same component")

  let list_fields x = x |> List.map Components.list_fields |> List.concat
  ...
  let print pr pol ppf t =
    ...
    | t, _ ->
        let pp_sep ppf () =
          Format.fprintf ppf "@ %s@ " (match pol with Pos -> "|" | Neg -> "&")
        in
...
let ty_list e loc =
  TCons (TypeLat.lift (Components.List (Location.one loc, e loc)))

let ty_fun d r loc =
  TCons (TypeLat.lift (Components.Func (Location.one loc, d loc, r loc)))
...
let ty_obj o loc =
  TCons
    (TypeLat.lift
       (Components.Object (SMap.map (fun x -> (Location.one loc, x loc)) o)))
...
and StateSet : (Intmap.S with type elt = State.state) = Intmap.Fake (struct
  type t = State.state

  let get_id = State.(fun s -> s.id)
end)
...
let fresh_id () =
  let n = !fresh_id_counter in
  incr fresh_id_counter;
  n
...
let compile_terms (map : (polarity -> var typeterm -> state) -> 'a) : 'a =
  ...
  let rec compile r p = function
    | TVar v -> (
        ...
        with Not_found ->
          ...
          StateTbl.add state_vars s v;
          s)
    | TCons c ->
        mkstate p
          (TypeLat.pmap (fun p t -> StateSet.singleton (compile r p t)) p c)
    | TAdd (t1, t2) ->
  ...
  let rec epsilon_closure set s =
    if StateSet.mem set s then set
      ...
      List.fold_left epsilon_closure (StateSet.add set s)
        (StateTbl.find_all epsilon_trans s)
  in
  let epsilon_merge s states =
    s.cons <-
      List.fold_left (state_cons_join s.pol) s.cons
        (List.map (fun s -> s.cons) states);
    List.iter
      (fun s' ->
        List.iter
          (fun v -> StateTbl.add state_vars s v)
          (StateTbl.find_all state_vars s'))
      states
  in
  List.iter
    (fun s ->
      epsilon_merge s (StateSet.to_list (epsilon_closure StateSet.empty s)))
    !states;
  ...
  let collect_constraint s v (vn, vp) =
    let ins map st var =
      VarMap.add var
        (StateSet.add
           (try VarMap.find var map with Not_found -> StateSet.empty)
           st)
        map
    in
    match s.pol with Pos -> (vn, ins vp s v) | Neg -> (ins vn s v, vp)
  in
  let var_state_neg, var_state_pos =
    StateTbl.fold collect_constraint state_vars (VarMap.empty, VarMap.empty)
  in
...
let find_reachable (roots : state list) =
  ...
  let rec search s acc =
    if StateTbl.mem states s then acc
    else (
      StateTbl.add states s ();
      s
      :: List.fold_right
           (fun (f, ss') acc ->
             StateSet.fold_left ss' acc (fun acc s' -> search s' acc))
           (TypeLat.list_fields s.cons)
           acc)
  in
...
let decompile_automaton (root : state) : var typeterm =
  ...
  let check () =
    StateSet.iter states (fun s ->
        StateSet.iter (StateTbl.find state_flow s) (fun s' ->
  ...
  let principal_biclique s =
    ...
    if StateSet.is_empty ss' then StateSet.(empty, empty)
    else
      let ss =
        StateSet.fold_left ss' states (fun m s' ->
            StateSet.inter (StateTbl.find state_flow s') m)
      in
      check ();
  ...
  let find_biclique () =
    List.fold_left
      ...
      (0, StateSet.empty, StateSet.empty)
      state_list
  in
    ...
    let del ss ss' =
      StateSet.iter ss (fun s ->
          StateTbl.replace state_flow s
            (StateSet.diff (StateTbl.find state_flow s) ss'))
    in
    check ();
    del ss ss';
    del ss' ss;
  ...
  let name_var id =
    if id < 26 then String.make 1 (Char.chr (Char.code 'a' + id))
    else Printf.sprintf "v_%d" (id - 26)
  in
  let fresh_var =
    let var_id = ref (-1) in
    fun () ->
  ...
  List.iter
    (fun (ss, ss') ->
      ...
      iter ss;
      iter ss')
    biclique_decomposition;

  let rec term_add p = function
    | [] -> TCons TypeLat.join_ident
    | [ t ] -> t
    | t :: ts ->
        if t = TCons TypeLat.join_ident then term_add p ts
        else TAdd (t, term_add p ts)
  in
  ...
  let rec decompile s =
    if StateTbl.mem state_rec_var s then (
      ...
      | None ->
          let v = fresh_var () in
          StateTbl.replace state_rec_var s (Some v);
          TVar v)
    else
...
let contraction sp_orig sn_orig =
  ...
  and closure_l ssp ssn =
    StateSet.fold_left ssp [] (fun rs sp ->
        rs @ StateSet.fold_left ssn rs (fun rs sn -> rs @ closure sp sn))
  in
...
let states_follow p (s : StateSet.t) : StateSet.t TypeLat.t =
  StateSet.fold_left s TypeLat.join_ident (fun c s ->
      TypeLat.join p (fun p a b -> StateSet.union a b) c s.cons)
...
and DStateSet : (Intmap.S with type elt = DState.dstate) = Intmap.Fake (struct
  type t = DState.dstate

  let get_id s = s.DState.d_id
end)
...
let clone f =
  ...
  let rec copy_state loc s =
    if DStateTbl.mem states s then DStateTbl.find states s
    else
      let s' =
        {
          id = fresh_id ();
      ...
      s'.cons <-
        TypeLat.pmap
          (fun pol d ->
            assert (pol = d.d_pol);
            StateSet.singleton (copy_state loc d))
          s.d_pol s.d_cons
        |> TypeLat.change_locations loc;
      s'.flow <-
        DStateSet.fold_left s.d_flow StateSet.empty (fun flow d ->
            StateSet.add flow (copy_state loc d));
      s'
  in
  f copy_state
...
let determinise old_states =
  ...
  let rec follow p s =
    if M.mem s !dstates then M.find s !dstates
    else
  ...
  in
  old_states
  |> List.iter (fun s -> ignore (follow s.pol (StateSet.singleton s)));
  (* flow edges:
  ...
  let dstates_containing =
    ...
    M.iter
      (fun ss d -> StateSet.iter ss (fun s -> StateTbl.add tbl s d))
      !dstates;
    fun s -> StateTbl.find_all tbl s
  in
  let flows_to a =
    StateSet.fold_left a.flow DStateSet.empty (fun ds s ->
        DStateSet.union ds (DStateSet.of_list (dstates_containing s)))
  in
  !dstates
  |> M.iter (fun ss d ->
         d.d_flow <-
           StateSet.fold_left ss DStateSet.empty (fun ds s ->
...
let minimise dstates =
  let rec check_disjoint s = function
    ...
    | p :: ps ->
        ...
        check_disjoint (DStateSet.union s p) ps
  in
...
*)
  ...
  let rec partition_list cmp acc = function
    ...
    | x :: xs ->
        ...
        partition_list cmp ((x :: same) :: acc) different
  in

  let rec repartition cmp = function
    | [] -> []
    | p :: ps -> partition_list cmp (repartition cmp ps) p
  in

  let same_ctor d d' =
    let sub_ctor d1 d2 =
      assert (d1.d_pol = d2.d_pol);
      TypeLat.subs d1.d_pol (fun pol x y -> true) d1.d_cons d2.d_cons
    in
    sub_ctor d d' && sub_ctor d' d
  in
  ...
  |> List.iter (fun d ->
         TypeLat.iter
           (fun p d' -> DStateTbl.add predecessors d' d)
           d.d_pol d.d_cons);
  ...
  let pred_ctor p ds' =
    let ds =
      DStateSet.(
        fold_left ds' empty (fun ds d' ->
            union ds (of_list (DStateTbl.find_all predecessors d'))))
    in
    DStateSet.fold_left ds TypeLat.join_ident (fun t d ->
        TypeLat.join p
          (fun p x y -> DStateSet.union x y)
          t
          (TypeLat.pmap
             (fun p d' ->
               if DStateSet.mem ds' d' then DStateSet.singleton d
               else DStateSet.empty)
             d.d_pol d.d_cons))
  in
  ...
  let rec split pol ds = function
    ...
    | p :: ps -> (
        ...
        | false, false ->
            let smaller = if length p1 < length p2 then p1 else p2 in
            active := smaller :: !active;
            p1 :: p2 :: split pol ds ps
        | false, true | true, false -> p :: split pol ds ps
        | true, true -> assert false)
    (* p should be nonempty *)
  in

  let rec partition_polarity p = (List.hd (DStateSet.to_list p)).d_pol in
  ...
  let rec refine ps =
    ...
    | ds :: rest ->
        ...
        refine (TypeLat.pfold split pol (pred_ctor pol ds) ps)
  in
  ...
  |> List.iter (fun (s, d) ->
         ...
         s.d_flow <-
           DStateSet.fold_left d.d_flow DStateSet.empty (fun flow x ->
               DStateSet.add flow (remap x)));
...
let rec entailed dn dp =
  ...
  if DStateSet.mem dn.d_flow dp then true
  else (
    dn.d_flow <- DStateSet.add dn.d_flow dp;
    dp.d_flow <- DStateSet.add dp.d_flow dn;
    if TypeLat.lte_np entailed dn.d_cons dp.d_cons then true
    else (
...
let subsumed map =
  ...
  let add sa db =
    ...
    if mem seen sa then
      ...
      if DStateSet.mem s db then true
      else (
  ...
  and subsume_all pol ssa db =
    StateSet.fold_left ssa true (fun r sa -> r && subsume pol sa db)
  in

  let all_entailed r dns dps =
    DStateSet.fold_left dns r (fun r dn ->
        DStateSet.fold_left dps r (fun r dp -> r && entailed dn dp))
  in
      ...
      (fun sa dbs r ->
        ...
        | Neg ->
            StateSet.fold_left sa.flow r (fun r sa' ->
                let dbs' =
                  try StateTbl.find seen sa' with Not_found -> DStateSet.empty
                in
                (* dbs <= sa <= sa' <= dbs' *)
                all_entailed r dbs dbs'))
      seen r
  in
